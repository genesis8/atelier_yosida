...
[mask_off time="400"]
;;; 文字サイズってどうやって調整すりゃいいんだろう？

;;; あと初期画面サイズも
;;; https://tyrano-complete.blogspot.com/2020/02/tyranoscript-window-size.html
;;; サイズの標準は960x640らしい
...
;;; [position]はメッセージウィンドウを指定するものっぽい。
;;; (0,440)-(960,640) が描画範囲、すなわち横は目いっぱい、縦は下から200。
;;; marigin top    50
;;;         left   25
;;;         right  25
;;;         bottom 10
;;; …これは下にメニューボタンを付けると余白足りなくなるのでは？

;;; ウィンドウIDみたいなものはないんだろうか？メッセージウィンドウはユニークなんだろうか？
;;;   → [layopt]で操作対象を指定できる。
;;;      [layopt layer="message0" visible="true"] みたいな。
...
;;; ここでデフォルトフォントを指定しているっぽい。
;;; 0xffffff=白
;;; 0x000000=黒
;;; で黒の縁取りがなされた白文字が出てくる
;;; ポスターデザイン的には縁取りなくても良くね？とは思う
...
;;; tf.image に格納される 'color/col1.png'、'color/col2.png' には、1ピクセルのカラーが入っている。
;;; それを引き延ばすことにより、810×90の枠を書いている。
;;; 5-95 100-190 195-285 290-380 … という感じで枠を作っていく。

;;; tf.index はどこに…と思ったら一番上で定義されていた。
;;; tf系の変数ってテンポラリーじゃないのか？
;;; →YES、だがこういう使い方もできるのか
...
;;; mouseopacity="50" color="white" でマウスかざすと透明度50％の白枠が重ねて表示されるのか
;;; [?]target="%target" ってなんだ？
;;;      →ジャンプ先のラベルを指定する
;;;      →説明書きにないだけで関数仕様としてはtargetタグを指定できるということか
;;; storage と target はどっちが優先されるんだ？
;;;      →どっちが、というよりは両方必要な感じだな
;;;      →storage 内にある target に飛ぶ、という感じ
;;;      →storage を省略すると現在シナリオ、 target を省略するとシナリオ先頭に飛ぶらしい
;;; 
;;; コメント長いから違和感あったけど実質4行で終わるマクロだなコレ
